...
class Table:
    ...
    def insert_entry(self, *values) -> bool:
        try:
            fields_string = ','.join([x.field_name for x in self.fields if x.field_name != 'id'])
    ...
    def update(self, column: str, value: str, id_column: str, id_value: str):
        self.cursor.execute("UPDATE %s SET %s = (%s) WHERE %s = %s", 
                            (self.table_name, column, value, id_column, id_value))

    def exists(self, key: str, value: str) -> bool:
        self.cursor.execute(f"SELECT * FROM {self.table_name} WHERE {key} = %s", (value,))
        return bool(self.cursor.fetchone())

    def fetch(self, property: str, key: str, value: str):
        self.cursor.execute(f"SELECT %s FROM {self.table_name} WHERE %s = %s", (property, key, value))
    ...
    def __initialize_table(self, fields: list[TableField]):
        ...
        self.cursor.execute(f"CREATE TABLE IF NOT EXISTS {self.table_name} ({field_string})")
